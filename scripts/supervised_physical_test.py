...
def compensate(theta_des):
    """
    input desired theta
    output theta commanded to achieve theta desired

    theta_command = f(theta_des)
    """
    # theta_command = f()
    theta_command = np.dot(np.asarray(theta_des), compensator_weights_1)
...
class RolloutExecuter(Trajectory):
    ...
    def __init__(self, sweep_joint_dict, debug=False, goal_type='API', state_type='OL'):
        ...
        self.state_keys = ['theta_commanded', 'theta_measured', 'velocity', 'torque', 'theta_desired']
    ...
    def init_state_vector(self):
        ...
        if self.debug:
            ...
            print("\n")
            print("Joint name keys: \n")
            print(self.robot_state_current['theta_commanded'].keys())
            print("\n")
    ...
    def make_local_goal(self, k):
        ...
        if self.goal_type == 'API':
            ...
            for name in self.robot_state_current['theta_desired'].keys():
    ...
    def goal_iteration(self):
        ...
        for idx in range(1,len(self._r_goal.trajectory.points) - 1):
